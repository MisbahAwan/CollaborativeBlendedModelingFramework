	       ...
	       else // executed when log is called
	       {
	    	  ...
	    	  // PrintWriter out = response.getWriter();
	    	   ...
	    	   for (int i = 0; i < rs.size(); i++)	           
				tableRows=tableRows+"<tr><td>"+rs.get(i).getSummaryAI()+"</td><td>"+rs.get(i).getLogin()+"</td><td>"+rs.get(i).getCreatedOn()+"</td></tr>";
	  ...
	  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //  ServletUtility.setSuccessMessage(" I AM IN POST", request);
	       ...
	       if(user != null) {
	           ...
	           if (user.getUserType().equals("web"))
	        	   ServletUtility.redirect("home.jsp", request, response);
	           else if (user.getUserType().equals("desktop"))
	        	   ServletUtility.redirect("homeDesktop.jsp", request, response);
